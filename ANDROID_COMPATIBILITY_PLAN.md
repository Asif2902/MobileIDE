...
| OpenCode Android command | **1.18.23 HOST/ABI VERIFIED — DEVICE RETEST** | pending 1.3.31 release commit | OpenCode 1.18.23 and matching OpenTUI 0.4.5 are rebuilt from exact upstream commits for Android/Bionic ARM64. The graph is PIE/16-KiB aligned and the renderer exports all 342 required symbols. The previous 1.17.9 payload passed version/help/debug/run/serve/web, TUI, spinner, and Bash-tool checks on API 30; the upgraded payload requires its phone retest. API 29/36 and x86_64 payload coverage remain. |
...
| OpenCode CLI | ⚠️ OpenCode 1.18.23 host/ABI verified; device/API matrix gate | The source-built OpenCode 1.18.23 ARM64 graph uses app-private temp, the ADEV shell broker, sibling APK-native ripgrep, and a matching source-built OpenTUI 0.4.5 Bionic library. It preserves upstream spinner registration and the non-fatal unknown-component fallback. Exact source patches, hashes, PIE, dependencies, 16-KiB alignment, and all 342 required renderer symbols pass host checks. The previous 1.17.9 runtime passed the real Bash-tool 22/22 offline and 23/23 network suite plus normal/forced-spinner TUI checks on API 30; the upgraded runtime and API 29/36 remain device gates. x86_64 has an explicit unavailable boundary; no glibc binary is substituted. |
