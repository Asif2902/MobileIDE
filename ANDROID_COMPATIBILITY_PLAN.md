...
| OpenCode Android command | **1.18.23 ARM64/API-30 TUI STARTUP VERIFIED — MATRIX GATE** | `ad38ea4` + `44daca2` | OpenCode 1.18.23 and matching OpenTUI 0.4.5 are rebuilt from exact upstream commits for Android/Bionic ARM64. The graph is PIE/16-KiB aligned and the renderer exports all 342 required symbols. The Android graph builder pins OpenTUI's parser worker to its explicit `bunfs` entry, fixing the `loadedPath.startsWith` startup crash. On-device instrumentation on Infinix X689B/API 30 proves the real TUI remains alive through startup and the execution contract still passes 22/22. API 29/36 and x86_64 payload coverage remain. |
...
| OpenCode CLI | ⚠️ OpenCode 1.18.23 ARM64/API-30 startup verified; remaining API matrix gate | The source-built OpenCode 1.18.23 ARM64 graph uses app-private temp, the ADEV shell broker, sibling APK-native ripgrep, and a matching source-built OpenTUI 0.4.5 Bionic library. It preserves upstream spinner registration and the non-fatal unknown-component fallback. Exact source patches, hashes, PIE, dependencies, 16-KiB alignment, and all 342 required renderer symbols pass host checks. A focused physical-device test proves the upgraded real TUI survives startup without the OpenTUI parser-worker `loadedPath.startsWith` crash, and the same build passes the 22/22 offline execution contract on API 30. API 29/36 remain device gates. x86_64 has an explicit unavailable boundary; no glibc binary is substituted. |
